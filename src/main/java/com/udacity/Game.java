...
public class Game {
    ...
    public String checkGameWinner(char[][] grid) {
        ...
        int rightDiagonalXCounter = 0;
        int tieFullFieldsCoutner = 0;
        int numberOfRowsAndCols = 3;

       for(int i=0; i<numberOfRowsAndCols;i++) {
           //checks diagonal winners
            if (grid[i][i] == 'o') {
                leftDiagonalOCounter++;
                if (leftDiagonalOCounter == numberOfRowsAndCols)
                    return "O wins";

            } else if (grid[i][i] == 'x') {
                leftDiagonalXCounter++;
                if (leftDiagonalXCounter == numberOfRowsAndCols)
                    return "X wins";

            }
            if (grid[numberOfRowsAndCols - 1 - i][i] == 'o') {
                rightDiagonalOCounter++;
                if (rightDiagonalOCounter == numberOfRowsAndCols)
                    return "O wins";

            } else if (grid[numberOfRowsAndCols - 1 - i][i] == 'x') {
                rightDiagonalXCounter++;
                if (rightDiagonalXCounter == numberOfRowsAndCols)
                    return "X wins";
            }
            //checks horizontal and vertical winners
            ...
            for (int j = 0; j < numberOfRowsAndCols; j++) {
                if (grid[i][j] == 'o' || grid[i][j] == 'x') tieFullFieldsCoutner++;
                if (tieFullFieldsCoutner == numberOfRowsAndCols * numberOfRowsAndCols) result = "Tie";
            }
        }
        return result;
    }
